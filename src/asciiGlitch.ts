...
const config = {
  ...
  mutationRatioMax: 0.095,
  lineShiftChance: 0.5,
}

const asciiGlitchCharset = Array.from({ length: 95 }, (_, index) => String.fromCharCode(32 + index)).join('')
const decayFrameMinMs = 24
const decayFrameMaxMs = 78

export function initBabylonAsciiGlitch(): () => void {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    return () => undefined
  }

  const hero = document.querySelector<HTMLElement>('.ascii-hero')
  const base = document.getElementById('babylon-ascii')
  const glitch = document.getElementById('babylon-ascii-glitch')
  ...
  function trigger() {
    ...
    idleTimeoutId = 0
    burstActive = true
    if (followupBudget === 0 && maybe(0.34)) followupBudget = randomInt(1, 3)
...
function sampleGlitchCharacter(source: string): string {
  if (/[0-9]/.test(source)) return String(Math.floor(Math.random() * 10))
...
function chooseWeightedDelays(min: number, max: number): number {
  ...
  if (roll < 0.16) return randomBetween(min, min + span * 0.18)
  if (roll < 0.72) return randomBetween(min + span * 0.28, min + span * 0.72)
...
function mutateLine(line: string, mutationRatio: number, start = 0, end = line.length): string {
  ...
  const mutableIndices = chars
    ...
    .map(({ index }) => index)
  if (mutableIndices.length === 0) return line
...
function shiftSegment(line: string, start: number, end: number): string {
  ...
  const shifted = offset > 0
    ...
    : `${segment.slice(Math.min(-offset, segment.length))}${' '.repeat(Math.min(-offset, segment.length))}`.slice(0, segment.length)
...
function corruptSegment(line: string, start: number, end: number, collapseBias = 0): string {
  ...
  const boundedStart = Math.max(0, Math.min(start, chars.length))
  const boundedEnd = Math.max(boundedStart, Math.min(end, chars.length))
  if (boundedEnd <= boundedStart) return line
  ...
  if (mode === 0) {
    ...
    for (let i = 0; i < count; i += 1) chars[Math.min(anchor + i, boundedEnd - 1)] = source
  } else if (mode === 2) {
...
function erodeSegment(line: string, start: number, end: number, strength: number): string {
  ...
  for (let index = boundedStart; index < boundedEnd; index += 1) {
    ...
    if (maybe(0.06 + strength * 0.2)) chars[index] = ' '
    else if (maybe(0.04 + strength * 0.12)) chars[index] = '.'
  }
  return chars.join('')
}
...
function collapseSegment(line: string, start: number, end: number, strength: number): string {
  ...
  const boundedEnd = Math.max(boundedStart, Math.min(end, chars.length))
  const segment = chars.slice(boundedStart, boundedEnd)
  if (segment.length === 0) return line
...
function distortZone(line: string, zone: GlitchZone, mutationRatio: number, lineShiftChance: number): string {
  ...
  if (maybe(lineShiftChance * (0.65 + zone.strength * 0.45))) next = shiftSegment(next, zone.colStart, zone.colEnd)
  if (maybe(0.42 + zone.strength * 0.4)) next = corruptSegment(next, zone.colStart, zone.colEnd, zone.collapseBias)
  if (maybe(0.24 + zone.strength * 0.3)) next = erodeSegment(next, zone.colStart, zone.colEnd, zone.strength)
  if (maybe(zone.collapseBias * 0.35)) next = collapseSegment(next, zone.colStart, zone.colEnd, zone.strength)
  return next
}

function createGlitchFrame(source: string, mutationRatio: number, lineShiftChance: number, zones: GlitchZone[]): string {
  return source.split('\n').map((line, rowIndex) => {
    let next = line
    for (const zone of zones) if (zoneAffectsRow(zone, rowIndex)) next = distortZone(next, zone, mutationRatio, lineShiftChance)
    return next
  }).join('\n')
}
...
function createBaseFrame(source: string, mutationRatio: number, lineShiftChance: number, zones: GlitchZone[]): string {
  return source.split('\n').map((line, rowIndex) => {
    ...
    for (const zone of zones) {
      if (!zoneAffectsRow(zone, rowIndex)) continue
      next = mutateLine(next, mutationRatio * (0.4 + zone.strength * 0.28), zone.colStart, zone.colEnd)
      if (maybe(lineShiftChance * 0.2)) next = shiftSegment(next, zone.colStart, zone.colEnd)
      if (maybe(0.14 + zone.strength * 0.18)) next = erodeSegment(next, zone.colStart, zone.colEnd, zone.strength * 0.9)
      if (maybe(zone.collapseBias * 0.18)) next = collapseSegment(next, zone.colStart, zone.colEnd, zone.strength * 0.7)
    }
    if (maybe(0.16) && next.trim()) {
      ...
      for (let i = 0; i < spliceLength; i += 1) {
        ...
        if (!chars[index] || /\s/.test(chars[index] ?? '')) continue
        chars[index] = sampleGlitchCharacter(chars[index])
      }
      next = chars.join('')
    }
...
function createDecayFrame(source: string, keepRatio: number): string {
  ...
  return lines.map((line, rowIndex) => {
    if (rowIndex < rowWindow.start || rowIndex >= rowWindow.end) return blankLine(line)
    return line.split('').map((char) => {
      if (/\s/.test(char)) return ' '
      if (maybe(keepRatio)) return maybe(0.28) ? sampleGlitchCharacter(char) : char
      return maybe(0.04) ? '.' : ' '
    }).join('')
  }).join('\n')
}
...
function applyGlitchVisualState(hero: HTMLElement): void {
  ...
  hero.style.setProperty('--ascii-scan-opacity', randomBetween(0.14, 0.34).toFixed(2))
  hero.style.setProperty('--ascii-glitch-opacity', randomBetween(0.42, 0.88).toFixed(2))
  hero.style.setProperty('--ascii-glitch-shift-x', `${randomInt(-10, 10)}px`)
  hero.style.setProperty('--ascii-glitch-shift-y', `${randomInt(-3, 3)}px`)
  hero.style.setProperty('--ascii-glitch-shadow-a', `${randomInt(1, 4)}px`)
  hero.style.setProperty('--ascii-glitch-shadow-b', `${randomInt(-4, -1)}px`)
  ...
  hero.style.setProperty('--ascii-bloom-opacity', randomBetween(0.015, 0.04).toFixed(2))
}
...
function applyGlitchDecayVisualState(hero: HTMLElement, remainingFrames: number): void {
  ...
  hero.style.setProperty('--ascii-scan-opacity', randomBetween(0.04, 0.14 * fade).toFixed(2))
  hero.style.setProperty('--ascii-glitch-opacity', randomBetween(0.08, 0.34 * fade).toFixed(2))
  hero.style.setProperty('--ascii-bloom-opacity', '0')
  hero.style.setProperty('--ascii-glitch-shift-x', `${randomInt(-4, 4)}px`)
  hero.style.setProperty('--ascii-glitch-shift-y', `${randomInt(-2, 2)}px`)
}

function clearGlitchVisualState(hero: HTMLElement): void {
  ;[
    '--ascii-scan-top', '--ascii-scan-height', '--ascii-scan-left', '--ascii-scan-width', '--ascii-scan-opacity',
    '--ascii-glitch-opacity', '--ascii-glitch-shift-x', '--ascii-glitch-shift-y', '--ascii-glitch-shadow-a',
    '--ascii-glitch-shadow-b', '--ascii-glitch-clip-top', '--ascii-glitch-clip-right', '--ascii-glitch-clip-bottom',
    '--ascii-glitch-clip-left', '--ascii-bloom-x', '--ascii-bloom-y', '--ascii-bloom-opacity',
